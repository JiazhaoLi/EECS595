...
def buildModel(train_feature, train_label):
    ...
    #input data...
    batch_size = 1
    train_feature_tensor = tf.placeholder(tf.float32, shape=[batch_size, 20, 100, 1], name='feature')
    ...
    unigram = tf.layers.conv2d(inputs=train_feature_tensor, filters=num_filter, kernel_size=(filter_size[0], embedding_dim), padding='same', activation=tf.nn.relu)
    bigram = tf.layers.conv2d(inputs=train_feature_tensor, filters=num_filter, kernel_size=(filter_size[1], embedding_dim), padding='same', activation=tf.nn.relu)
    trigram = tf.layers.conv2d(inputs=train_feature_tensor, filters=num_filter, kernel_size=(filter_size[2], embedding_dim), padding='same', activation=tf.nn.relu)
   
    unigram_f = tf.reshape(unigram[:,:,num_filter//2,:],shape=[-1,seq_len,num_filter])
    bigram_f = tf.reshape(unigram[:,:,num_filter//2,:],shape=[-1,seq_len,num_filter])
    trigram_f = tf.reshape(unigram[:,:,num_filter//2,:],shape=[-1,seq_len,num_filter])
    #attentive layer...
    # print(np.shape(train_feature))
    # print(tf.shape(train_feature_tensor))

    unigram_vec = unigram_f[:, 0, :]
    bigram_vec = bigram_f[:, 0, :]
    trigram_vec = trigram_f[:, 0, :]
    
    # print(tf.multiply(unigram_vec, weights['attention_W']))
    ...
    flatten_layer = tf.reshape(concat_layer,shape = (seq_len * embedding_dim,-1))
    ...
    for e in range(train_epoch):
        for i in range(len(train_feature)//batch_size - 1):
            ...
            # print(train_feature_oneBatch.shape)
            # print(train_label_oneBatch.shape)
...
def _intialize_weights(batch_size,num_filter,seq_len,embedding_dim):
    ...
    # weights['attention_feature'] = tf.zeros(shape=[batch_size, sample_len, num_filter])
